...
var mainModule = function() {
    ...
    var highlightCurrentCategory = function() {
        ...
        if (foundItem && $(foundItem).length > 0) $(foundItem).addClass("current")
    };
    ...
    var setHomepageImageSizes = function() {
        ...
        if (homepageCurrentWindowWidth > homepageWindowWidth && homepageWindowWidth > 768) return;
        ...
        for (var i = 0; i < $imgCollection.length; i++) {
            ...
            if (!$imgItem.hasClass("range-product")) {
                if ($imgItem.css("background-image") != undefined && $imgItem.css("background-image") != "none") {
                    ...
                    if ($imgSrcStr == undefined || $imgSrcStr.length == 0) $imgSrcStr = $imgItem.attr("img-src");
                    var $imgWidth = $imgItem.width();
                    for (var k = 0; k < imageSizeBreakdownArray.length; k++) {
                        if ($imgWidth < imageSizeBreakdownArray[k]) {
    ...
    var setRangeImages = function() {
        ...
        if (rangeCurrentWindowWidth > rangeWindowWidth) return;
        var newSize = "700";
    ...
    var setSocialVariables = function() {
        ...
        if ($(".buy-off-container").length != 0 && typeof currentObjId !== "undefined") rtnObj.socialUrlProdcode = currentObjId;
        else rtnObj.socialUrlProdcode = $("#productId").attr("value");
        ...
        if ($(".buy-off-container").length != 0 && typeof currentObjId !== "undefined") $(".twitter-social").attr("href", "//twitter.com/share?url=" + rtnObj.socialUrlShare + "&text=" + rtnObj.socialProductDescription + "%20by%20TOAST:%20");
        return rtnObj
    };
    ...
    if ($("#toastFont").length > 0) {
        ...
        $(window).scroll(function() {
            ...
            if (sd > 500) $("#more-content-arrow").fadeOut(300);
            else $("#more-content-arrow").fadeIn(300)
        })
    }
    $(document).ready(function() {
            ...
            if ($(this).find(".new-sub-nav li").length > 0) {
                ...
                $(this).find(".new-sub-nav .main-link, .new-sub-nav .new-sub-nav-list-heading").each(function() {
                    ...
                    if ($(this).hasClass("new-sub-nav-list-heading")) {
                        thisSubNav += "<li class='nav-option' id=" + $(this).text()+"><a href='#' class='mobile-sub-list-heading'>+ " + $(this).text() + "</a></li>";
    ...
    if ($("#toastFont, .homepageContainer").length > 0) {
        $(".landing-tile-content").each(function(index) {
            if ($(this).css("background-image") !== undefined && $(this).css("background-image") !== "none") landingBgdImgElemCollection.push($(this))
        });
...
var helpModule = function() {
    ...
    var toggleOpenAccordionSection = function(sectionId, tabNumber) {
        ...
        if (tabNumber == 0) $(".shopping-accordion").accordionA("toggle", "#" + sectionId, true);
        else if (tabNumber == 1) $(".legal-accordion").accordionA("toggle", "#" + sectionId, true);
        else if (tabNumber == 2) $(".company-accordion").accordionA("toggle", "#" + sectionId, true);
        setTimeout(function() {
            $("body, html").animate({
    ...
    $(document).ready(function() {
        ...
        if (helpSectionId != "") {
            sectionNumber = 0;
            if ($("#" + helpSectionId).parent().hasClass("legal-accordion")) sectionNumber = 1;
            else if ($("#" + helpSectionId).parent().hasClass("company-accordion")) sectionNumber = 2;
...
var rangeModule = function() {
    ...
    var initAddToBasket = function() {
        $(".add-to-bag").addtobasket({
            ...
            addToBasketSuccessHandler: function(button, parameters) {
                ...
                button.parent().find("#added-to-basket").show().html("<span class='message-text'>" + parameters.productDisplayName + ", size " + size + " has been added to your basket.</span>");
                if (miniBasket) {
                    miniBasket.data("tcplMinibasket").refreshBasket()
                }
                document.body.style.cursor = "default";
                            ...
                            for (var b = 0; b < itemArr.length; b++) {
                                for (var k = 0; k < stockJson.stocklist.length; k++) {
                                    if (stockJson.stocklist[k].prodId.toLowerCase() == itemArr[b].prodId.toLowerCase()) {
                                        ...
                                        if (!itemInStock && colourMode) inStockArr.push(itemArr[b]);
                                        break
                                    }
                                }
                            }
                            ...
                            for (var m = itemArr.length - 1; m >= 0; m--) {
                                if (itemArr[m].baseColour.toLowerCase() === baseColour) {
                                    ...
                                    if (false) {
                                        ...
                                        var scalingImageFileName = "/" + prop + "_" + itemObj.colour.replace(/ /g, "").replace(/\//g, "") + ".jpg";
                                        imgSrc = "/stormsites/toast/images/range-images/" + categoryStr + scalingImageFileName
                                    } else imgSrc = itemObj.upImg.replace(sizeRegex, "/250/");
                                    if (colourMode) {
                                        $($("#" + itemObj.prodId + " img")[0]).attr({
                                            src: imgSrc.replace("gift_clear", "gift/clear"),
    ...
    var populateRangeSizesBasedOnStock = function() {
        ...
        if (!bedLinenMode && currentIndividualObj) {
            for (var i = 0; i < currentIndividualObj.skuSetArr.length; i++) {
                ...
                if (sizeDisplay.toLowerCase() == "os") sizeDisplay = "One Size";
                if (inStock) {
                        ...
                        if (prop != "title" && prop != "description" && prop != "details" && prop != "fitNotes" && prop != "provenance") {
                            ...
                            for (var p = 0; p < workingObj.length; p++) {
                                ...
                                if ($(".selected-swatch").attr("base-colour").toLowerCase() == workingObj[p].baseColour.toLowerCase() && $(".selected-swatch").attr("title").toLowerCase() == workingObj[p].colour.toLowerCase() || $(".selected-swatch").attr("alt").toLowerCase() == workingObj[p].colour.toLowerCase()) {
                                    if (!sizeDisplay || sizeDisplay.length == 0) sizeDisplay = "Undetermined";
                                    for (var k = 0; k < stockJson.stocklist.length; k++) {
        ...
        if ($(".product-sizes .size").length == $(".product-sizes .out-of-stock").length) $(".add-to-bag").attr("disabled", "disabled").text("Out of Stock");
        else $(".add-to-bag").removeAttr("disabled").text("Add To Bag")
    };
    ...
    var populateRangeBuyOffSection = function() {
        ...
        if (typeof parsedHtmlDesc == "undefined" || parsedHtmlDesc.length == 0) parsedHtmlDesc = currentObjDescription;
    ...
    var setRangeItemQuantityEnabled = function() {
        if ($(".product-sizes li.size-selected").length > 0) {
            if ($(".product-sizes li.size-selected").hasClass("size-out-of-stock")) $(".qtyList").attr("disabled", "disabled");
            else $(".qtyList").removeAttr("disabled")
        }
    };
    ...
    var setRangeImage = function(calledFromSwatch) {
        ...
        if (currentWindowWidth > windowWidth && calledFromLightbox === false) return;
        ...
        if ($img.attr("src") != undefined && $img.attr("src").search(sizeRegex) != -1) {
            ...
            if (globalIndex != -1) str = str.replace("/global/", "/product/");
    ...
    addRangeListeners = function() {
        ...
        $(".group-buy-off").on("click", ".size", function(e) {
            if ($(this).hasClass(".size-selected")) return;
            if (bedLinenMode) {
        ...
        $(".colour-tiles li").click(function() {
            if (!$(this).hasClass("selected-colour-tile")) {
                if ($(".selected-colour-tile").length > 0) $(".selected-colour-tile").removeClass("selected-colour-tile");
                if ($(".selected-mix-tile").length > 0) $(".selected-mix-tile").removeClass("selected-mix-tile");
...
var wishlistModule = function() {
                    ...
                    if (prop != "title" && prop != "description" && prop != "details" && prop != "fitNotes" && prop != "provenance") {
                        for (var j = 0; j < moduleVar.wishlistProductJson.products[i][prop].length; j++) {
                            ...
                            if (addTemplate) {
                                ...
                                if ($newItem.find(".sale-price").length > 0) $newItem.find(".price").addClass("strike-through");
                                break
                            }
                        }
                    }
                    if (addTemplate) break
                }
                if (addTemplate) break
            }
        }
    ...
    var setWishlistEventListeners = function() {
        ...
        $(".wishlist-edit-btn").click(function(e) {
            ...
            var modFigure = 4;
            if (window.innerWidth <= 1024 && window.innerWidth > 767) modFigure = 3;
            else if (window.innerWidth < 768) modFigure = 1;
            while (editBtnIndex >= countWishItems) {
                countWishItems += modFigure;
                if (editBtnIndex + 1 <= countWishItems) {
                    if (countWishItems <= $(".wishlist-edit-btn").length) $($(".wishlist-product")[countWishItems - 1]).after($(".buy-off-container"));
                    else $($(".wishlist-product")[$(".wishlist-edit-btn").length - 1]).after($(".buy-off-container"))
                }
            }
            setTimeout(function() {
                ...
                if (moduleVar.wishListCollection.wishlist[i].skuCode.toLowerCase() == selectedItemSku.toLowerCase()) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if ($(".wishlist-product").length == 0) $(".wishlist-product-group").html('<p class="empty-wishlist-text">You&#39;ve got nothing on your wishlist. Find things to add to it <a href="/">here</a></p>')
                        }
                    });
                    break
                }
    ...
    var getWishlistItemTemplate = function(parentItem, item, prop, matchedItem) {
        ...
        } else newPrice = parseFloat(item.price).toFixed(2).replace(".00", "");
        ...
        if (typeof parsedHtmlDesc == "undefined" || parsedHtmlDesc.length == 0) parsedHtmlDesc = parentItem.description;
        var wishItemSizeStr = "";
        for (var i = 0; i < moduleVar.wishlistStockJson.stocklist.length; i++) {
                ...
                for (var k = 0; k < moduleVar.wishlistStockJson.stocklist[i].sizesInStock.length; k++) {
                    ...
                    if (matchedItem.skuCode.toLowerCase() == moduleVar.wishlistStockJson.stocklist[i].sizesInStock[k].sku.toLowerCase()) {
                        ...
                        if (itemSizesObj.preOrderAvailable != "") {
                            ...
                            if (itemSizesObj.stlev == 0) addToBagStr = wishItemOutOfStockStr
                        }
        ...
        $("#updateItemWishlist").click(function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (selectedQuantity == 0 || selectedQuantity == "0") selectedQuantity = "1";
                    ...
                    if (stockLevel == 0 && isPreOrderAvailable) $updatedItem.find(".wishlist-add-to-bag-btn span").text(wishItemPreOrderAvailableStr);
                    else if (stockLevel == 0 && !isPreOrderAvailable) $updatedItem.find(".wishlist-add-to-bag-btn span").text(wishItemOutOfStockStr);
                    else $updatedItem.find(".wishlist-add-to-bag-btn span").text(wishItemAddToBagStr);
    ...
    var setupDeleteFromWishlistButton = function() {
        ...
        if (!itemInWishlist) visuallyHiddenStr = "visuallyHidden";
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.d.Success) {
                        ...
                        var prodTitle = $(".product-details").find(".product-info h1").text() || $(".product-details").find(".product-info h3").text();
                        var returnToProductURL = "/login.htm?returnUrl=";
                        returnToProductURL = returnToProductURL + document.location.pathname.replace(/\//gi, "%2f").replace(/\./gi, "%2E") + "?addProdToWishList=true";
                        window.location.href = returnToProductURL;
                        if (typeof prodTitle !== "undefined") {
    ...
    var checkItemInWishlist = function() {
        ...
        if (typeof thisProdCode == "undefined") thisProdCode = $(".buy-off-container .product-details").attr("prod-id");
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if ($(".product-details").length > 0) {
                        ...
                        setTimeout(function() {
                            if (addProdOnLoad) $("#addToWishlist").trigger("click")
                        }, 1e3)
                    }
                }
            })
                ...
                if (!data.HasErrors) {
                    if (data.TotalResults === 0) {
                        ...
                        for (var i = 0; i < data.Results.length; i++) {
                            ...
                            if (data.Results[i].Rating > bestRating) bestRating = data.Results[i].Rating
                        }
                        ...
                        for (var i = 0; i < data.Results.length; i++) {
                            ...
                            for (var j = 0; j < data.Results[i].SecondaryRatingsOrder.length; j++) {
                                if (data.Results[i].SecondaryRatings[data.Results[i].SecondaryRatingsOrder[j]].ValueLabel !== null) addSecondarySection(data.Results[i].SecondaryRatings[data.Results[i].SecondaryRatingsOrder[j]], $newReview)
                            }
